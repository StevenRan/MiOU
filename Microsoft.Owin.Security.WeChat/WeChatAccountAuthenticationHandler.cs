...
namespace Microsoft.Owin.Security.WeChat
{
    internal class WeChatAccountAuthenticationHandler : AuthenticationHandler<WeChatAuthenticationOptions>
    {
        ...
        private async Task<bool> InvokeReturnPathAsync()
        {
            ...
            var authTicket = await AuthenticateAsync();
            if(authTicket==null)
            {
                _logger.WriteVerbose("No auth ticket");
                return false;
            }
            var context = new WeChatReturnEndpointContext(Context, authTicket);
            context.SignInAsAuthenticationType = Options.SignInAsAuthenticationType;
            context.RedirectUri = authTicket.Properties.RedirectUri;
			authTicket.Properties.RedirectUri = null;
        ...
        protected override async Task<AuthenticationTicket> AuthenticateCoreAsync()
        {
            ...
            try
            {
                ...
                context.Identity = new ClaimsIdentity(new[]{
                    ...
                    new Claim("urn:wechatconnect:city", context.City,XmlSchemaString,Options.AuthenticationType),
                    new Claim("urn:wechatconnect:image", context.AvtorUrl,XmlSchemaString,Options.AuthenticationType),
